...
class Model(object):
    ...
    def vggnet(self):
        # 读取预训练的vgg模型
        vgg = scipy.io.loadmat(config.VGG_MODEL_PATH)
        ...
        net['input'] = tf.Variable(np.zeros([1, config.IMAGE_HEIGHT, config.IMAGE_WIDTH, 3]), dtype=tf.float32)
        # 参数对应的层数可以参考vgg模型图
    ...
    def get_random_img(self):
        ...
        :return:
        """
        noise_image = np.random.uniform(-20, 20, [1, config.IMAGE_HEIGHT, config.IMAGE_WIDTH, 3])
        random_img = noise_image * config.NOISE + self.content * (1 - config.NOISE)
        return random_img
    ...
    def loadimg(self, path):
        ...
        # 减去均值，使其数据分布接近0
        image = image - config.IMAGE_MEAN_VALUE
        return image


if __name__ == '__main__':
    Model(config.CONTENT_IMAGE, config.STYLE_IMAGE)
