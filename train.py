...
def loss(sess, model):
    ...
    content_layers = config.CONTENT_LOSS_LAYERS
    ...
    # 再计算风格损失函数
    style_layers = config.STYLE_LOSS_LAYERS
...
def train():
    # 创建一个模型
    model = models.Model(config.CONTENT_IMAGE, config.STYLE_IMAGE)
    # 创建session
    with tf.Session() as sess:
        ...
        for step in range(config.TRAIN_STEPS):
            ...
            if step % 50 == 0:
                ...
                # 训练过程是减去均值的，这里要加上
                img += config.IMAGE_MEAN_VALUE
                # 这里是一个batch_size=1的batch，所以img[0]才是图片内容
                img = img[0]
                # 将像素值限定在0-255，并转为整型
                img = np.clip(img, 0, 255).astype(np.uint8)
                # 保存图片
                scipy.misc.imsave('{}{}.jpg'.format(config.OUTPUT_IMAGE, step), img)
        # 保存最终训练结果
        img = sess.run(model.net['input'])
        img += config.IMAGE_MEAN_VALUE
        img = img[0]
        img = np.clip(img, 0, 255).astype(np.uint8)
        scipy.misc.imsave(('{}{}.jpg'.format(config.OUTPUT_IMAGE, 'result')), img)
